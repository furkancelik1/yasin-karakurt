...
import { Utensils, FileText, Download, Calendar, Clock } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-display text-white uppercase tracking-wide">
            Beslenme <span className="text-gold italic">Planı</span>
          </h1>
        ...
        <div className="rounded-2xl border border-white/5 bg-charcoal/40 p-12 text-center">
          <div className="w-16 h-16 rounded-full bg-gold/10 border border-gold/20 flex items-center justify-center mx-auto mb-4">
            <Utensils size={28} className="text-gold/50" />
          </div>
          ...
          <p className="text-ash/30 text-sm mt-2">
            Yasin Hoca beslenme planınızı hazırladığında burada görünecek.
          </p>
        </div>
      </div>
    );
      ...
      >
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold tracking-widest uppercase text-gold/60">Aktif Plan</p>
          ...
          <div className="flex items-center gap-2 text-ash/40 text-xs">
            <Calendar size={12} />
            <span>{new Date(nutritionProgram.createdAt).toLocaleDateString('tr-TR')}</span>
          </div>
        </div>

        {nutritionProgram.contentType === 'FILE' && nutritionProgram.fileUrl ? (
          <div className="p-6 bg-white/[0.02] rounded-xl border border-white/5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <FileText size={20} className="text-gold/70" />
                <span className="text-ash/70">Beslenme Dosyası</span>
              </div>
              <a
                href={`${API_URL}${nutritionProgram.fileUrl}`}
                target="_blank"
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-ash/40 text-xs pt-2">
              <Clock size={12} />
              <span>Son güncelleme: {new Date(nutritionProgram.updatedAt).toLocaleString('tr-TR')}</span>
            </div>
          </div>
        ) : (
          <p className="text-ash/40 italic">Beslenme planı içeriği henüz eklenmemiş.</p>
        )}
      </motion.div>
    </div>
  );
